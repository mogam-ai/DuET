...
# Citation: TBD
#
# run_cds_attention_streme.sh - STREME on CDS attention local-peak k-mers
# vs a position-matched control set.
#
# Inputs (from attribution_score_and_attention.ipynb CDS attention-export cell):
#   data/cds_attn_peak.fa     primary: attention-spike-centered k-mers
#   data/cds_attn_control.fa  control: position-matched non-peak sites
# Usage:  conda activate motif; bash run_cds_attention_streme.sh
set -euo pipefail

# ---- config ----------------------------------------------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DATA="${SCRIPT_DIR}/data"
OUT="${SCRIPT_DIR}/results/cds_attention_streme"
PEAK="${DATA}/cds_attn_peak.fa"
CTRL="${DATA}/cds_attn_control.fa"
...
BG="${TMPDIR}/cds_attn_ctrl_bg.txt"
