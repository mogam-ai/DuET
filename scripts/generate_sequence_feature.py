...
from duet.data.utils import CODON_TO_AA, codon_weight
...
def fold_LP(seq: str) -> tuple[str, float]:
    ...
    # LinearPartition binary: override via LINEARPARTITION_BIN, else expect it on PATH.
    lp_bin = os.environ.get("LINEARPARTITION_BIN", "linearpartition")
    command = (f"echo {seq} | "
               f"{lp_bin} "
...
def get_AUG_context_stats(seqs: str, radius: int=30):
    ...
    # ddG (unconstrained vs AUG-context-constrained folding) is intentionally not computed.
