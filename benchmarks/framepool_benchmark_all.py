...
def framepool_benchmark(dataset_name = 'U_1',
                        data_df = None,
                        rank_split=True,
  ...
  if data_df is None: raise ValueError("data_dict must be provided")

  # Encoding functions
  one_hot_fn = utils_data.OneHotEncoder("utr5")
  out_encoding_fn = utils_data.DataFrameExtractor("te", method="direct")
  ...
  train_generator = utils_data.DataSequence(train_data,
                                            encoding_functions=[one_hot_fn],
                                            output_encoding_fn=out_encoding_fn,
                                            shuffle=True)
  val_generator = utils_data.DataSequence(val_data,
                                          encoding_functions=[one_hot_fn],
                                          output_encoding_fn=out_encoding_fn,
                                          shuffle=False)
  test_generator = utils_data.DataSequence(test_data,
                                           encoding_functions=[one_hot_fn],
                                           output_encoding_fn=out_encoding_fn,
                                           shuffle=False)
  ...
  # Keras porting - from_generator 우회
  # TypeError: `output_signature` must contain objects that are subclass of `tf.TypeSpec` but found <class 'list'> which is not.
  import tensorflow as tf

  # 샘플 가져오기
  sample_input, sample_label = train_generator[0]
  print("Sample input shape:", sample_input.shape)
  print("Sample label shape:", sample_label.shape)

  def generator_wrapper():
      while True:
          # epoch 시작 전마다 shuffle.
          train_generator.on_epoch_end()
          try:
              for inputs, labels in train_generator:
                  yield inputs, labels
          except IndexError:
              continue # IndexError가 발생하면 다음 epoch

  output_signature = (
      tf.TensorSpec(shape=(None, None, 4), dtype=tf.float32),  # x1: variable-length sequence
      #(
      #    #tf.TensorSpec(shape=(None, None, 4), dtype=tf.float32),  # x1: variable-length sequence
      #    #tf.TensorSpec(shape=(None, 2), dtype=tf.float32),         # x2: fixed
      #),
      tf.TensorSpec(shape=(None,), dtype=tf.float32)               # y: fixed
  )
  ...
  # load the best model
  utr_model = load_model(model_save_path,
        custom_objects={
          'FrameSliceLayer': model.FrameSliceLayer,
          'compute_pad_mask': model.compute_pad_mask,
          'apply_pad_mask': model.apply_pad_mask,
          'global_avg_pool_masked': model.global_avg_pool_masked,
          'PrintShapeLayer': model.PrintShapeLayer,
          'interaction_term': model.interaction_term,
          })


  predict = utr_model.predict(test_generator, verbose=0)
  y = test_data["te"].values.reshape(-1)
  yhat = predict.reshape(-1)
  ...
  all_results_path = '../Benchmark/benchmark_results.tsv'
...
def load_benchmark_data_all(dataset_type='MRL_merged'):
  full_df = []
  def safe_float(x):
      try: return float(x)
  ...
  for dataset in datasets:
    ...
    # we apply standard scaling to te
...
full_df = load_benchmark_data_all()
full_df

# %%
#framepool_benchmark(dataset_name='HEK',
#                    data_df=full_df,
#                    rank_split=False,
#                    split_num=0, # ignored if rank_split is true
#                    max_epochs=50,
...
for dataset_name in ['U_1', 'U_2', 'm1pU_1', 'm1pU_2', 'mC-U_1', 'mC-U_2', 'pU_1', 'pU_2']:
    ...
    framepool_benchmark(dataset_name=dataset_name,
                        data_df=full_df,
                        rank_split=True,
                        split_num=0, # ignored if rank_split is true
                        max_epochs=15,
