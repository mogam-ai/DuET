...
#from concise import encodeRNAStructure
...
class SecondaryStructureComputer(PrecomputeFunction):
    ...
    def __call__(self, df):
        #return encodeRNAStructure(seq_vec, maxlen=None, seq_align='end', W=240, L=160, U=1, tmpdir='/tmp/RNAplfold/')
        return None
...
class DataSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # Feed input
        # 🔧 핵심 수정: list → tuple (단일이면 그대로)
        inputs = tuple(fn(batch_df) for fn in self.encoding_functions)

        if len(inputs) == 1:
            inputs = inputs[0] # 튜플이 아니라 텐서만
