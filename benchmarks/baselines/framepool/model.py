...
class FrameSliceLayer(Layer):
    ...
    def call(self, x):
        shape = tf.shape(x)
        x = tf.reverse(x, axis=[1]) # reverse, so that frameness is related to fixed point (start codon)
        frame_1 = tf.gather(x, tf.range(start=0, limit=shape[1], delta=3), axis=1)
        frame_2 = tf.gather(x, tf.range(start=1, limit=shape[1], delta=3), axis=1)
        frame_3 = tf.gather(x, tf.range(start=2, limit=shape[1], delta=3), axis=1)
...
def global_avg_pool_masked(input_tensors):
    ...
    mask = input_tensors[1]
    mask = tf.expand_dims(mask, axis=2)
    return tf.reduce_sum(tensor, axis=1)/tf.reduce_sum(mask, axis=1)
...
def convolve_and_mask(conv_features, pad_mask, n_filters, kernel_size, suffix, prefix="",
    ...
    conv_features = Lambda(apply_pad_mask, output_shape=lambda s:s[0], name=prefix+"apply_pad_mask_"+suffix)([conv_features, pad_mask]) # Mask padding
    if batchnorm:
...
def create_frame_slice_model(n_conv_layers=3,
    ...
    pad_mask = Lambda(compute_pad_mask, output_shape=lambda s: s[:2], name="compute_pad_mask")(conv_features)
    # Hasan-track
    if use_counter_input:
        input_counter = Input(shape=(None, ), name="input_counter")
        inputs = [input_seq, input_counter]
        counter = Lambda(lambda x: tf.expand_dims(x, axis=2), output_shape=lambda s: (s[0], s[1], 1), name="dim_expand")(input_counter)
    ...
    avg_pooling = Lambda(global_avg_pool_masked, output_shape=lambda s: (s[0][0], s[0][2]), name="pool_avg_frame_conv")
    ...
    pooled_features = PrintShapeLayer(name="print_shape_after_pooling")(pooled_features) # print shape layer JH add
    ...
    if use_scaling_regression:
        ...
        predict = Lambda(interaction_term, output_shape=lambda s: s[1], name="interaction_term")([predict, input_experiment])
...
def create_model_recurrent(n_conv_layers=3,
    ...
    if frame_indicator:
        ...
        masked_frame = Lambda(apply_pad_mask, output_shape=lambda s:s[0], name="apply_pad_mask_frame")([input_frame, pad_mask])
    ...
    if kozak_indicator:
        ...
        masked_kozak = Lambda(apply_pad_mask, output_shape=lambda s:s[0], name="apply_pad_mask_kozak")([input_kozak, pad_mask])
...
def create_standard_conv_model(n_conv_layers=3,
    ...
    """ Model """
    model = Model(inputs=inputs, outputs=predict)
    #adam = keras.optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08) lr deprecated
    adam = keras.optimizers.Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    model.compile(loss=loss, optimizer=adam)
    return model