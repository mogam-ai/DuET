...
def seq2tensor(seq: str, max_len: int) -> torch.Tensor:
  ...
  if len(ids) > max_len: ids = ids[-max_len:]        # truncate: keep last max_len
  ids = ids + [PAD_TOKEN] * (max_len - len(ids))     # pad to max_len
...
def tensorize(seq: str,
              ...
              **kwargs) -> torch.Tensor:
    """One-hot encode a sequence (no padding).

    Pass quantizer=lambda x: [x[k:k+3] for k in range(0, len(x), 3)] and default=64
    to encode codons instead of bases.
    """
    onehot_tensor = torch.tensor(seq2ids(code, seq, **kwargs), dtype=torch.long)
    onehot_tensor = F.one_hot(onehot_tensor, num_classes=max(code.values())+1)
    return onehot_tensor[:, :channel_size].float()
